...
class DetectionService {
    async detectTechnologies(codebasePath) {
        const technologies = [];
        ...
        const packageJsonPath = path_1.default.join(codebasePath, 'package.json');
        try {
        ...
        const requirementsPath = path_1.default.join(codebasePath, 'requirements.txt');
        try {
        ...
        // Check for Streamlit
        const hasStreamlit = await this.searchInFiles(codebasePath, /import\s+streamlit|from\s+streamlit/, 'py');
    ...
    async detectPythonVersion(codebasePath) {
        // Check .python-version file
        try {
            const pythonVersionFile = path_1.default.join(codebasePath, '.python-version');
            const version = (await promises_1.default.readFile(pythonVersionFile, 'utf-8')).trim();
            return version;
        }
        catch {
            // Not found
        ...
        try {
            const pyprojectPath = path_1.default.join(codebasePath, 'pyproject.toml');
        ...
        try {
            const runtimePath = path_1.default.join(codebasePath, 'runtime.txt');
            const content = (await promises_1.default.readFile(runtimePath, 'utf-8')).trim();
    ...
    async detectPyprojectDependencies(codebasePath, technologies) {
        try {
            const pyprojectPath = path_1.default.join(codebasePath, 'pyproject.toml');
            const content = await promises_1.default.readFile(pyprojectPath, 'utf-8');
    ...
    async searchInFiles(dir, pattern, fileType) {
        try {
    ...
    async getCodeFiles(dir, fileType) {
        const files = [];
    ...
    getTechnologySeverity(name) {
        const cursed = ['jquery', 'bower', 'tslint'];
        const haunted = ['moment', 'request', 'grunt', 'gulp'];
        if (cursed.includes(name.toLowerCase()))
            return 'cursed';
        if (haunted.includes(name.toLowerCase()))
            return 'haunted';
        return 'clean';
    }
}
